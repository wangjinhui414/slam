...
namespace MYTOOLS
{
	void CalStringToArray(char* str, char*restr,CGroup<int> &varry);                  //‘,’间隔数字字符串转数组
	CGroup<string>  CalString(char* str, const char* ch);
	string inttostring(int iin);
	int stringtoint(string str);
	double stringtodouble(string str);
	void StartTimeCount(DWORD &timer);
	DWORD EndTimeCount(DWORD timer);
};
